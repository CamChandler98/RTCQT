...
namespace Audio
{
	...
	float FPseudoConstantQ::GetConstantQCenterFrequency(const int32 InBandIndex, const float InBaseFrequency, const float InBandsPerOctave , float LogBase)
	{
	...
	float FPseudoConstantQ::GetStupidConstantQCenterFrequency(const int32 InBandIndex, const float InBaseFrequency, const float InBandsPerOctave,  float LogBase, int32 MaxBands)
	{
		...
		if(LogBase){
			float denom = -((MaxBands - (static_cast<float>(InBandIndex) / InBandsPerOctave)) - InBandIndex);
			float scale = FMath::Pow(LogBase, (denom/InBandsPerOctave));
